...
@Component({ selector: 'app-productos', imports: [RouterLink], templateUrl: './productos.component.html', styleUrl: './productos.component.css' })
export class ProductosComponent {
  ...
  async cargarProductos(): Promise<void> {
    ...
    try { this.productos.set(await this.productoService.obtenerProductos()); }
    catch { this.mensajeError.set('No fue posible cargar los productos.'); }
    finally { this.cargando.set(false); }
  }

  actualizarBusqueda(evento: Event): void { this.busqueda.set((evento.target as HTMLInputElement).value); }
  actualizarCategoria(evento: Event): void { this.categoria.set((evento.target as HTMLSelectElement).value); }
  actualizarEstado(evento: Event): void { this.estado.set((evento.target as HTMLSelectElement).value as FiltroEstado); }
  solicitarCambioEstado(producto: Producto): void { this.productoConfirmacion.set(producto); this.mensajeExito.set(''); }

  async confirmarCambioEstado(): Promise<void> {
    const producto = this.productoConfirmacion();
    if (!producto || this.cambiandoEstado()) return;
    this.cambiandoEstado.set(true); this.mensajeError.set('');
    try {
      const actualizado = await this.productoService.cambiarEstado(producto.id, !producto.activo);
      this.productos.update((productos) => productos.map((actual) => actual.id === actualizado.id ? actualizado : actual));
      this.mensajeExito.set(actualizado.activo ? 'El producto fue reactivado.' : 'El producto fue desactivado.');
      this.productoConfirmacion.set(null);
    } catch { this.mensajeError.set('No fue posible cambiar el estado del producto.'); }
    finally { this.cambiandoEstado.set(false); }
  }
