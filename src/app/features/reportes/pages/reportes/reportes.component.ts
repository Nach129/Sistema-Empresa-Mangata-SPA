...
import {
  ...
  formatearClp,
  rangoFechasValido,
} from '../../utils/reporte-utilidades';

@Component({
  selector: 'app-reportes',
  imports: [
    ReactiveFormsModule,
    GraficoReporteComponent,
    IndicadoresReporteComponent,
    SelectorProductoReporteComponent,
  ],
...
export class ReportesComponent {
  ...
  readonly coloresEstados = ['#294c60', '#c58b37', '#d97745', '#7a9a89', '#3d7a5e', '#b85252'];
  readonly formulario = new FormGroup({
    desde: new FormControl<string | null>(null),
    hasta: new FormControl<string | null>(null),
    estado: new FormControl<EstadoPedido | null>(null),
  ...
  readonly estadosCompletos = computed(() =>
    completarEstados(this.reporte()?.pedidosPorEstado ?? []),
  );
  readonly etiquetasEstados = computed(() =>
    this.estadosCompletos().map((item) => etiquetaEstado(item.estado)),
  );
  readonly valoresEstados = computed(() => this.estadosCompletos().map((item) => item.cantidad));
  readonly etiquetasProductos = computed(
    () => this.reporte()?.productosMasSolicitados.map((item) => item.nombre) ?? [],
  );
  readonly valoresProductos = computed(
    () => this.reporte()?.productosMasSolicitados.map((item) => item.cantidad) ?? [],
  );
  readonly etiquetasVentas = computed(
    () => this.reporte()?.ventasPorMes.map((item) => etiquetaPeriodo(item.periodo)) ?? [],
  );
  readonly valoresVentas = computed(
    () => this.reporte()?.ventasPorMes.map((item) => item.total) ?? [],
  );
  readonly formatearClp = formatearClp;
  readonly etiquetaEstado = etiquetaEstado;

  constructor() {
    void this.cargar(this.filtros());
  }

  async aplicar(): Promise<void> {
    const filtros = this.filtros();
    if (!rangoFechasValido(filtros.desde, filtros.hasta)) {
      this.errorFiltros.set('La fecha desde no puede ser posterior a la fecha hasta.');
      return;
    }
    this.errorFiltros.set('');
    await this.cargar(filtros);
  }

  async limpiar(): Promise<void> {
    this.formulario.reset({ desde: null, hasta: null, estado: null, productoId: null });
    this.errorFiltros.set('');
    await this.cargar(this.filtros());
  }

  seleccionarProducto(productoId: number | null): void {
    this.formulario.controls.productoId.setValue(productoId);
  }

  abrirModalPdf(): void {
    const hoy = new Date();
    this.formularioPdf.reset({
      desde: this.fechaIsoLocal(new Date(hoy.getFullYear(), hoy.getMonth(), 1)),
      hasta: this.fechaIsoLocal(hoy),
    });
    this.errorPdf.set('');
  ...
  async generarPdf(): Promise<void> {
    ...
    if (this.formularioPdf.invalid) {
      this.formularioPdf.markAllAsTouched();
      this.errorPdf.set('Selecciona una fecha desde y una fecha hasta.');
      return;
    }
    if (!rangoFechasValido(desde, hasta)) {
      this.errorPdf.set('La fecha desde no puede ser posterior a la fecha hasta.');
      return;
    }
    this.generandoPdf.set(true);
    this.errorPdf.set('');
    try {
      const reportePdf = await this.servicio.obtenerReporte({
        desde,
        hasta,
        estado: null,
        productoId: null,
      });
      this.servicioPdf.descargar(reportePdf, desde, hasta);
      this.modalPdfAbierto.set(false);
    } catch {
      this.errorPdf.set('No fue posible generar el reporte PDF.');
    } finally {
  ...
  cerrarModalConEscape(): void {
    if (this.modalPdfAbierto()) this.cerrarModalPdf();
  }

  private filtros(): FiltrosReporte {
    const valor = this.formulario.getRawValue();
    return {
      desde: valor.desde || null,
      hasta: valor.hasta || null,
      estado: valor.estado || null,
      productoId: valor.productoId ? Number(valor.productoId) : null,
    };
  }
  ...
  private async cargar(filtros: FiltrosReporte): Promise<void> {
    ...
    this.error.set('');
    try {
      this.reporte.set(await this.servicio.obtenerReporte(filtros));
    } catch {
      this.error.set('No fue posible cargar los reportes.');
    } finally {
