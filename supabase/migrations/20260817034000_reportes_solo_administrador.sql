...
begin
  if not public.es_administrador() then raise exception 'NO_AUTORIZADO'; end if;
...
comment on function public.obtener_reporte_general(date,date,varchar,bigint) is
  'Entrega indicadores y estadísticas únicamente a usuarios con rol ADMINISTRADOR.';
